...
def baswana_sen_spanner(original_graph: Graph, stretch: int, weight_attribute: Optional[str] = None, random_seed: Optional[int] = 1, *, max_iteration_size_factor: float = 2.0,):
    spanner_graph = Graph()
    ...
    vertex_to_cluster_center = _phase1_form_clusters(spanner_graph, residual_graph, vertex_to_cluster_center, k_parameter, original_graph.number_of_nodes(), max_iteration_size_factor, random.Random(random_seed), weight_attribute)
...
def _phase1_form_clusters(spanner_graph: Graph, residual_graph: Graph, vertex_to_cluster_center: Dict[Hashable, Hashable], k_parameter: int, num_vertices: int, max_iteration_size_factor: float, random_generator: random.Random, weight_attribute: Optional[str]) -> Dict[Hashable, Hashable]:
    ...
    for iteration_number in range(max(0, k_parameter - 1)):
        vertex_to_cluster_center = _execute_single_clustering_iteration(spanner_graph, residual_graph, vertex_to_cluster_center, sampling_probability, iteration_edge_capacity, random_generator, weight_attribute)
    return vertex_to_cluster_center

def _execute_single_clustering_iteration(spanner_graph: Graph, residual_graph: Graph, current_clustering: Dict[Hashable, Hashable], sampling_probability: float, iteration_edge_capacity: int, random_generator: random.Random, weight_attribute: Optional[str]) -> Dict[Hashable, Hashable]:
    while True:
        ...
        if len(edges_to_add_to_spanner) <= iteration_edge_capacity:
            for vertex_u, vertex_v in edges_to_add_to_spanner:
                _add_edge_to_spanner(spanner_graph, residual_graph, vertex_u, vertex_v, weight_attribute)
            ...
            # Remove vertices not in clustering
            for vertex in list(residual_graph.nodes()):
...
def _step2_find_lightest_edges_to_clusters(residual_graph: Graph, vertex_to_cluster_center: Dict[Hashable, Hashable], source_vertex: Hashable,) -> Tuple[Dict[Hashable, Hashable], Dict[Hashable, Tuple]]:
...
def _handle_case_vertex_not_adjacent_to_sampled_clusters(vertex: Hashable, residual_graph: Graph, lightest_edge_to_cluster: Dict[Hashable, Hashable], edges_to_add_to_spanner: set[Tuple[Hashable, Hashable]], edges_to_remove_from_residual: set[Tuple[Hashable, Hashable]]):
...
def _handle_case_vertex_adjacent_to_sampled_clusters(
    ...
    edges_to_add_to_spanner.add(_convert_to_undirected_edge(vertex, lightest_edge_to_cluster[closest_sampled_cluster]))
    ...
    for neighbor in list(residual_graph.neighbors(vertex)):
        if current_clustering[neighbor] in lightest_weight_to_cluster and lightest_weight_to_cluster[current_clustering[neighbor]] <= lightest_weight_to_cluster[closest_sampled_cluster]:
            edges_to_remove_from_residual.add(_convert_to_undirected_edge(vertex, neighbor))

def _step4_remove_intra_cluster_edges(residual_graph: Graph, vertex_to_cluster_center: Dict[Hashable, Hashable]):
...
def _phase2_vertex_cluster_joining(spanner_graph: Graph, residual_graph: Graph, vertex_to_cluster_center: Dict[Hashable, Hashable], weight_attribute: Optional[str]):
    for vertex in list(residual_graph.nodes()):
        ...
        for neighbor_vertex in lightest_edge_to_cluster.values():
            _add_edge_to_spanner(spanner_graph, residual_graph, vertex, neighbor_vertex, weight_attribute)

def _create_residual_graph_with_edge_keys(original_graph: Graph, weight_attribute: Optional[str]):
    residual_graph = Graph()
...
def _add_edge_to_spanner(spanner_graph: Graph, residual_graph: Graph, vertex_u: Hashable, vertex_v: Hashable, weight_attribute: Optional[str],):
...
def create_graph():
    ...
    stretch = int(input("Enter stretch factor: ").strip())
    random_seed = int(input("Enter random seed: ").strip())
    return original_graph, random_seed, stretch

def main():
    original_graph, random_seed, stretch = create_graph()
    spanner_graph = baswana_sen_spanner(original_graph, stretch=stretch, weight_attribute="w", random_seed=random_seed)
    ...
    print(f"k = {max(1, (stretch + 1) // 2)} | Edge reduction: {100.0 * (1.0 - spanner_graph.number_of_edges() / max(1, original_graph.number_of_edges())):.2f}%")
